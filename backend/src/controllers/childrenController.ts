...
export const createChild = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    ...
    const existing = await prisma.child.findFirst({
      where: { parent_id: parentId, name: { equals: name, mode: 'insensitive' } }
    });

    if (existing) {
      res.status(400).json({ error: 'Já existe uma criança cadastrada com este nome.' });
      return;
    }
...
export const deleteChild = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    const parentId = req.user?.id;
    ...
    await prisma.$transaction([
      // 1. Delete SubTaskCompletions first (linked to TaskExecutions)
      prisma.subTaskCompletion.deleteMany({
        where: {
          execution: { assignment: { child_id: childId } }
        }
      }),
      // 2. Delete TaskExecutions (linked to TaskAssignments)
      prisma.taskExecution.deleteMany({
        where: {
          assignment: { child_id: childId }
        }
      }),
      // 3. Delete TaskAssignments
      prisma.taskAssignment.deleteMany({
        where: { child_id: childId }
      }),
      // 4. Delete RewardRedemptions
      prisma.rewardRedemption.deleteMany({
        where: { child_id: childId }
      }),
      // 5. Finally, delete the Child itself
      prisma.child.delete({
        where: { id: childId as string },
      }),
    ]);
