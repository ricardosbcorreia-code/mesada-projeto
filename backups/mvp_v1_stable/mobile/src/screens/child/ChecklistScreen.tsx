  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      >
        ...
        {(() => {
          const renderTask = (exec: Execution) => {
              ...
              >
                ...
                <View style={styles.taskContent}>
                  ...
                  <Text style={[styles.taskValue, isDone && { color: icon.color }, exec.assignment?.task?.type === 'penalty' && { color: Colors.danger }]}>
                    {isDone ? statusLabel[exec.status] : `${exec.assignment?.task?.type === 'penalty' ? '-' : '+'}${Number(exec.assignment?.task?.value ?? 0).toFixed(2)} pts`}
                  </Text>
                </View>
                {!isDone && (
